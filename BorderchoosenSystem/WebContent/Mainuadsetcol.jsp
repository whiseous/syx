...
<body>
		<div id="wrapper">
			...
			<div id="container">
				...
				<div id="main">
				<form action="Uniadmin.action?id=${uadmin.uid}" method="post" name="uadseupcol" id="uadseupcol">
				<input type=hidden name="opttype" value="uadreuptutor">
					<br></br>
				<font size="4">欢迎您！管理员：${uadmin.uid}，以下为相关院级管理员信息</font><br></br>
				<input type="submit" value="添加院管理">
				<br></br>
					<div id="mainboard">
					<br></br>
						<div id="board">
								...
								<tbody>
									<c:forEach var="c" items="${clist}" varStatus="status">
								       <tr bgcolor="${status.index % 2 == 1 ? 'FFFFFF' : 'F0F0F0'}">
								         ...
								         <td>${c.tsub}</td>
								         <td>
								            <a href='Uniadmin.action?id=${uadmin.uid}&cid=${c.tid}&opttype=delecol'>删除</a>
								            <a href='Uniadmin.action?id=${uadmin.uid}&cid=${c.tid}&opttype=viewcol'>查看详细</a>
								         </td>
								       </tr>
								    </c:forEach>
								</tbody>
